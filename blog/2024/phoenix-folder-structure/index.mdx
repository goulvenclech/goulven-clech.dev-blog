...
On [its doc](https://hexdocs.pm/phoenix/directory_structure.html), Phoenix recommends a very simple directory structure, mainly to start with and mainly for the front end.

Why it took so long -> At the start of the start-up, and after the departure of the company's first dev, I remained alone for a long time with difficulty recruiting. A work-study student and a web integrator joined the team, and sometimes we worked with freelancers, but without other developers and not having that much experience, I tried to keep the initial philosophy of the project as long as possible.
-> This is one of the first big decisions I made, and I wanted to make it right. 
...
encheres_immo/
    ...
    - accounts/
        - user.ex (child schema)
        - organization.ex (child schema)
        - [...utilis]
...
Three context examples, all with different structures. Making the overall file very complex to understand.

Additionally, some of these structures simply don't work. For example, the context `accounts.ex` includes both functions involving multiple child schemas, and functions that should be in a context specific to each of these child schemas. The result is an abnormally long file, with a messy organization.
